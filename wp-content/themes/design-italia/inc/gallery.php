...
function property_gallery_save( $post_id ) {
	...
	// Correct post type
	if ( 'operaduepercento' != $_POST['post_type'] ) // here you can set the post type name
		return;
	...
	if ( $_POST['gallery'] ){
		...
		for ($i = 0; $i < count( $_POST['gallery']['image_url'] ); $i++ ){
			if ( '' != $_POST['gallery']['image_url'][$i]){
				...
				$gallery_data['image_alt'][]  = $_POST['gallery']['image_alt'][ $i ];
				$gallery_data['image_title'][]  = $_POST['gallery']['image_title'][ $i ];
				$gallery_data['image_caption'][]  = $_POST['gallery']['image_caption'][ $i ];
			}
		}
...
function property_gallery_add_metabox(){
	add_meta_box(
		...
		'operaduepercento', // Change post type name
		'normal',
		'high',
        null
	);
			...
			<div class="gallery_single_row dolu">
			  <div class="gallery_area image_container ">
				...
				<input type="hidden"
                  ...
                  <input type="hidden"
						 class="meta_image_url"
						 name="gallery[image_alt][]"
						 value="<?php esc_html_e( $gallery_data['image_alt'][$i] ); ?>"
				  />
                  <input type="hidden"
						 class="meta_image_url"
						 name="gallery[image_title][]"
						 value="<?php esc_html_e( $gallery_data['image_title'][$i] ); ?>"
				  />
                  <input type="hidden"
						 class="meta_image_url"
						 name="gallery[image_caption][]"
						 value="<?php esc_html_e( $gallery_data['image_caption'][$i] ); ?>"
				  />
			  </div>
		...
		<div style="display:none" id="master_box">
			<div class="gallery_single_row">
				<div class="gallery_area image_container" onclick="open_media_uploader_image(this)">
					...
					<input class="meta_image_alt" value="" type="hidden" name="gallery[image_alt][]" />
					<input class="meta_image_title" value="" type="hidden" name="gallery[image_title][]" />
					<input class="meta_image_caption" value="" type="hidden" name="gallery[image_caption][]" />
				</div> 
				<div class="gallery_area"> 
					<span class="button remove" onclick="remove_img(this)" title="Remove"/><i class="fas fa-trash-alt"></i></span>
				</div>
...
function property_gallery_styles_scripts(){
    global $post;
    if( 'operaduepercento' != $post->post_type )
        return;
    ?>  
    <style type="text/css">
	.gallery_area {
	...
	function open_media_uploader_image(obj){
		...
		media_uploader.on("insert", function(){
			...
			jQuery(obj).find('.meta_image_alt').val(image_alt);
			jQuery(obj).find('.meta_image_title').val(image_title);
			jQuery(obj).find('.meta_image_caption').val(image_caption);
		});
		media_uploader.open();
	}
	function open_media_uploader_image_this(obj){
		...
		media_uploader.on("insert", function(){
			...
			jQuery(obj).siblings('.meta_image_alt').val(image_alt);
			jQuery(obj).siblings('.meta_image_title').val(image_title);
			jQuery(obj).siblings('.meta_image_caption').val(image_caption);
		});
		media_uploader.open();
	}
	...
	function open_media_uploader_image_plus(){
		...
		media_uploader.on("insert", function(){
			...
			for(var i = 0; i < length; i++){
                ...
                var image_alt = images[i].changed.alt; 
				...
				element.find('.meta_image_alt').val(image_alt);
				element.find('.meta_image_title').val(image_title);
				element.find('.meta_image_caption').val(image_caption);
...
add_action( 'admin_head-edit.php', 'property_gallery_styles_scripts' );